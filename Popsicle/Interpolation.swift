...
open class Interpolation<T: Interpolable> : Equatable, ObjectReferable, Timeable {
	let object: NSObject
	let keyPath: String
	...
	fileprivate var poles: [Time: Pole] = [:]
	...
	public init<U: NSObject>(_ object: U, _ keyPath: KeyPath<U, T>) {
		...
		if !self.object.responds(to: NSSelectorFromString(self.keyPath)) {
			assertionFailure("Please make sure the key path \"" + self.keyPath + "\" you're referring to for an object of type <" + NSStringFromClass(type(of: self.object)) + "> is invalid")
		}
	}
	...
	open func setEasingFunction(_ easingFunction: @escaping EasingFunction, forTime time: Time) {
		self.poles[time]?.1 = easingFunction
	}

	open subscript(time1: Time, rest: Time...) -> T.ValueType? {
		get {
